...
"""
此脚本中将词汇是否出现作为特征，与出现次数的多少无关，
为 词集模型（set-of-words model)
"""
...
def classifyNaiveBayes(vec2classify, p0vec, p1vec, pclass1):
	...
	"""
	p1 = np.sum(vec2classify * p1vec) + np.log(pclass1) #依据条件独立性假设，从概率向量中取值出来，利用了文档向量的0-1特性
...
def testing_NaiveBayes():
	...
	for postingDoc in postinglist:
		trainmat.append(setofWords2Vec(myvocablist, postingDoc)) #将文档依据词汇表转换为文档矩阵
	p0v, p1v, pAb = trainNaiveBayes(trainmat, labels) #计算条件概率，已取对数

	testentry = ['love', 'my', 'dalmation'] #测试文档
	thisDoc = setofWords2Vec(myvocablist, testentry) #依据词汇表将文档转为文档向量
